...
from asrun.common.sysutils import is_localhost, same_hosts
...
def is_localhost2(host, ignore_domain=True, user=""):
    ...
    if not result and magic.run is not None:
        ...
        magic.log.debug(u'localhost %s (%s) : %s', local_hostid, hostid, result)
        magic.log.debug(u'is_localhost %s (%s) : %s', host, hostid, result)
    return result
...
def same_hosts2(host1, host2, user1='', user2=''):
    ...
    if not result and magic.run is not None:
        ...
        magic.log.debug(u"are '%s' and '%s' the same host ? %s", host1, host2, result)
    return result
...
def get_hostid(host='', user=''):
    ...
    if hostid is None and not _cache_hostid.has_key(key):
        hostid = _get_remote_hostid(host, user)
        _cache_hostid[key] = hostid
    magic.log.debug(u"hostid('%s')=%s using user '%s'" % (host, hostid, user))
    return hostid
...
def _get_remote_hostid(host, user):
    ...
    if CMD['hostid'] == 'hostid':
        ...
        magic.log.debug(u"##host = %s, user = %s, iret = %d" % (host,user,iret))
        if iret == 0:
            readdr = re.compile('HWaddr *([0-9:a-z]+)', re.I)
        ...
        #peter.zhang for remote connection
            ...
            iret, out = magic.run.Shell('ipconfig /all', mach=host, user=user)
            if iret == 0:
                readdr = re.compile(': *([0-9\-a-z]+\-[0-9\-a-z]+)', re.I)
                mat = readdr.search(out, re.I)
                if mat:
                    hostid = mat.group(1)
