    ...
    opts_descr = {
        ...
        'exectool' : {
            ...
            'kwargs' : {
                ...
                'default' : False,
                'dest'    : 'exectool',
                'help'    : _(u'run using the specified tool (for --quick/--test action)')
            }
        },
        'run_params' : {
            'args'   : ('--run_params', ),
            'kwargs' : {
                ...
                'default' : [],
                'dest'    : 'run_params',
                'help'    : _(u'list of parameters added in the export file '
...
def StartTestNow(run, *args):
    ...
    if not run.get('aster_vers'):
        run.parser.error(_(u"You must define 'default_vers' in 'aster' configuration file or use '--vers' option."))
    if len(args) > 2:
        run.parser.error(_(u"'--%s' requires at most two arguments") % run.current_action)
    testcase = args[0]
    resutest = None
    if len(args) > 1:
        resutest = args[1]
