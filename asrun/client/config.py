...
from asrun.common.sysutils import get_home_directory, local_user
...
class ClientConfig(object):
    ...
    def check_rcdir(self):
        ...
        try:
            ...
            self._rcdir = self._rcdir.replace('\\', '/')
            self._rcdir = self._rcdir.replace('/cygdrive/', '')
            if self._rcdir.find(':/') < 0:
                self._rcdir = self._rcdir.replace('/', ':/', 1)
    ...
    def get_server_config(self, server, use_ip=False):
        ...
        if use_ip:
            dname = dict([(ds["nom_complet"], key) for key, ds in self._serv.items()])
    ...
    def refresh_server_config(self, server_list=None):
        ...
        for server in server_list:
            ...
            print "refresh_server_config############",schema_name
            print "refresh_server_config############",prof
            print "refresh_server_config############",server_list
