...
from asrun.build        import AsterBuild, getDiag
...
def SetParser(run):
    ...
    acts_descr = {
        ...
        'showme'        : {
            ...
            'help'   : _(u'Print informations about installation and configuration')
        },
    }
    opts_descr = {
        'local' : {
...
def get_aster_version(vers, error=True):
    ...
    if use_waf:
        ...
        result = map(str, pkg[0]) + [pkg[3], pkg[2].startswith('v'), rev]
    else:
...
def AutoUpdate(run, *args):
    ...
    """
    run.parser.error(_(u"'--%s' is not supported anymore, see "
...
def MakeDiag(run, *args):
    ...
    if not l_dirs:
        run.parser.error(_(u"'--astest_dir' option is required"))
...
""")
    ...
    # get the dict result
    dict_resu = getDiagnostic(run, l_dirs, list_tests)
...
def get_diagnostic(run, build, test_dirs, list_tests):
    ...
    warn("'get_diagnostic(run, build, test_dirs, list_tests)' is deprecated " \
         "and replaced by 'getDiagnostic(run, test_dirs, list_tests)'.",
         DeprecationWarning, stacklevel=2)
    return getDiagnostic(run, test_dirs, list_tests)


def getDiagnostic(run, test_dirs, list_tests):
    """Return the diagnosis of the execution of a list of testcases
    """
    list_tests.sort()
    nbtest = len(list_tests)
    ...
    for test in list_tests:
        ...
        diag, tcpu, tsys, ttot, telap = getDiag(run, err=ferre, resu=fresu, \
                                                mess=fmess, cas_test=True)
        dict_resu[test] = {
            'test'  : test,
            'diag'  : diag,
            'tcpu'  : tcpu,
...
def ShowMe(run, *args):
    ...
    run.PrintExitCode = False
    if len(args) < 1:
        run.parser.error(_(u"'--%s' requires at leat one argument") % run.current_action)
    what = args[0]
    if what in ('bin', 'lib', 'etc', 'data', 'locale') and len(args) != 1:
        run.parser.error(_(u"'--%s %s' requires one argument") \
                         % (run.current_action, what))
    if what in ('param', ) and len(args) != 2:
        run.parser.error(_(u"'--%s %s' requires two arguments") \
                         % (run.current_action, what))
    if what == 'bin':
        print3(aster_root)
    elif what == 'lib':
        ...
        print3(osp.join(get_home_directory(), run.rcdir))
    elif what == 'data':
        print3(datadir)
    elif what == 'locale':
        print3(localedir)
