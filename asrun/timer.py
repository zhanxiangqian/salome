...
class AsterTimer:
    ...
    def __init__(self, add_total=True, format='as_run', maxlabel=None):
        ...
        if format == 'as_run':
            self.mintitr = 33
            self._fmtlig = u'   %%(name)-%(adjtitr)ds  %%(cpu_dt)9.2f  %%(sys_dt)9.2f  %%(cpu_sys)9.2f  %%(tot_dt)9.2f'
            self._fmtstr = u'   %%(title)-%(adjtitr)ds  %%(cpu)9s  %%(sys)9s  %%(cpu+sys)9s  %%(elapsed)9s'
            self._sepa   = ' ' + '-'*81
            self.TotalKey = u'Total'  # DO NOT translate this label because it will be "grep'ed" by AsterCalcul
            self.d_labels = {
                'title'   : '',
                'cpu'     : _(u'cpu'),
                'sys'     : _(u'system'),
            ...
            self.mintitr = 24
            self._fmtlig = u' * %%(name)-%(adjtitr)ds : %%(cpu_dt)10.2f : %%(sys_dt)10.2f : %%(cpu_sys)10.2f : %%(tot_dt)10.2f *'
            self._fmtstr = u' * %%(title)-%(adjtitr)ds : %%(cpu)10s : %%(sys)10s : %%(cpu+sys)10s : %%(elapsed)10s *'
            self._sepa   = ' ' + '*'*80
            self.TotalKey = u'TOTAL_JOB'
            self.d_labels = {
                'title'   : u'COMMAND',
                'cpu'     : u'USER',
                'sys'     : u'SYSTEM',
    ...
    def adjust_format(self):
        ...
        """
        larg_max = max([0,] + [len(timer['name']) for timer in self.timers.values() \
