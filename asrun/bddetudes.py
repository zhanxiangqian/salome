...
def SetParser(run):
    ...
    acts_descr = {
        'insert_in_db' : {
            'method' : Insert,
            'syntax' : 'export_file',
            'help'   : _(u'prepare the profile to insert an execution into a database.')
        },
    }
    #opts_descr = {}
    title = _(u'Options for astketud database link')
    run.SetActions(
...
def Insert(run, *args):
    ...
    if not run.config.get('astketud'):
        run.Mess(_(u"'astketud' is not defined in 'agla' configuration file."), '<F>_AGLA_ERROR')
