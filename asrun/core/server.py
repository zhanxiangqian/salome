...
class BaseServer(object):
    ...
    def __init__(self, host, user, **kwargs):
        ...
        magic.log.info("BaseServer init  host=%s   user=%s", self.host, self.user)
...
class ExecServer(BaseServer):
    ...
    def exec_command(self, command, display_forwarding=False, **opts):
        ...
        if type(cmd) in (list, tuple):
            cmd = ' '.join(cmd)
        magic.log.info(_(u"execute on %s@%s : %s"), self.user, self.host, cmd)
...
class CopyServer(BaseServer):
    ...
    def __init__(self, host, user, **kwargs):
        ...
        self.proxy_dir = self.proxy_dir.replace('\\','/')
...
class CopyToServer(CopyServer):
    ...
    def copyto(self, *files, **opts):
        ...
        ret = 0
        if len(files) == 0:
            magic.log.info(_(u"no file to copy."))
        for src in files:
...
class LocalCopyServer(CopyToServer, CopyFromServer):
    ...
    def _create_dir(self, directory):
        """Create a directory on the server."""
        magic.log.info(_(u"create directory %s..."), directory)
        try:
            #peter.zhang, for cygwin
            directory = directory.replace('\\', '/')
            directory = directory.replace('/cygdrive/', '')
            if directory.find(':/') < 0:
                directory = directory.replace('/', ':/', 1)
            os.makedirs(directory)
        except:
            pass
        magic.log.info(_(u"done"))
    ...
    def delete_proxy_dir(self):
        ...
        #peter.zhang, for cygwin
        self.proxy_dir = self.proxy_dir.replace('\\', '/')
        self.proxy_dir = self.proxy_dir.replace('/cygdrive/c', 'c:')
        magic.log.info(_(u"delete directory %s..."), self.proxy_dir)
        try:
            shutil.rmtree(self.proxy_dir)
        except OSError:
            magic.log.info(_(u"failed"))
        else:
            magic.log.info(_(u"done"))
    ...
    def _copyoneto(self, src, convert=None):
        ...
        src = src.replace('\\', '/')
        self.proxy_dir = self.proxy_dir.replace('\\', '/')
        magic.log.info(_(u"copy %s to %s"), src, self.proxy_dir)
...
def build_server_from_profile(prof, type, **kwargs):
    ...
    assert proto, u"'%s' not defined in the profile" % KEYS[type]
    if type in (TYPES.COPY_TO, TYPES.COPY_FROM):
        kwargs['proxy_dir'] = prof['proxy_dir'][0]
        assert kwargs['proxy_dir'], u"'proxy_dir' not defined in the profile"
    if type == TYPES.EXEC:
        kwargs['aster_root'] = prof['aster_root'][0]
        assert kwargs['aster_root'], u"'aster_root' not defined in the profile"
