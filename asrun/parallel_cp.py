...
class PARALLEL_COPY:
    ...
    def __init__(self, tracker, master, dest, leech_limit, verbose=False, run=None):
        ...
        if not os.path.exists(self.tracker):
            try:
                ...
                self.tracke = self.tracke.replace('\\', '/')
                self.tracke = self.tracke.replace('/cygdrive/', '')
                if self.tracke.find(':/') < 0:
                    self.tracke = self.tracke.replace('/', ':/', 1)
    ...
    def start(self):
        ...
        if not os.path.exists(self.dest):
            ...
            self.dest = self.dest.replace('/cygdrive/', '')
            if self.dest.find(':/') < 0:
                self.dest = self.dest.replace('/', ':/', 1)
