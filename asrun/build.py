...
class AsterBuild_Classic:
    ...
    def Compil(self, typ, rep, repobj, dbg, rep_trav='',
        ...
        #peter.zhang, for cygwin
        repobj = repobj.replace('\\', '/')
        repobj = repobj.replace('/cygdrive/', '')
        if repobj.find(':/') < 0:
            repobj = repobj.replace('/', ':/', 1)
    ...
    def CompilAster(self, REPREF, repdest='', dbg='nodebug', ignore_ferm=False,
        ...
        lmods = [d for d in para.keys() if d != '']
    ...
    def Link(self, exe, lobj, libaster, libferm, reptrav):
        ...
        if obj.find(':/') < 0:
            obj = obj.replace('/', ':/', 1)
        exe = exe.replace('\\', '/')
        exe = exe.replace('/cygdrive/', '')
        if exe.find(':/') < 0:
            exe = exe.replace('/', ':/', 1)
        rep = rep.replace('\\', '/')
        rep = rep.replace('/cygdrive/', '')
        if rep.find(':/') < 0:
            rep = rep.replace('/', ':/', 1)
    ...
    def PrepEnv(self, REPREF, repdest, dbg='nodebug', lang='', **kargs):
        ...
        self._check_file_args(kargs.values())
        ...
        # ----- copy Cygwin libs
        iret = 0
        src  = osp.join(REPREF, '*.dll')
        dest = osp.join(repdest, repdest)
        ...
        if use_symlink and not self.run.IsRemote(src):
            ...
            if len(l_capy) == 0 and lang != '':
                ...
                l_capy = glob(osp.join(src, mask_catapy))
            if len(l_capy) == 0:
                self._mess(ufmt(_(u'no catalogue found : %s'), osp.join(src, mask_catapy)),
                        '<F>_FILE_NOT_FOUND')
            # add symlink
            for f in l_capy:
...
""" % self.conf['REPPY'][0]
            ...
            if i18n:
                ...
                for lang in [l for l in self.conf['I18N'] if not l in ('', 'fr')]:
                    ...
                    # fake aster module
                    open(osp.join(self.conf['REPPY'][0], 'aster.py'), 'w').write("""
# fake aster module
""")
...
""")
    ...
    def GetUnigest(self, funig):
        ...
        """
        dico = {}
        print "--------------------------------- aster build.py GetUnigest : ", funig
        #peter.zhang, for cygwin
        funig = funig.replace('\\', '/')
    ...
    def GetDiag(self, *args, **kwargs):
        ...
        """
        warn("'AsterBuild.GetDiag(...)' is deprecated and replaced by " \
             "the function 'getDiag(run, ...)'.",
             DeprecationWarning, stacklevel=2)
        return getDiag(self.run, *args, **kwargs)
    ...
    def _check_file_args(self, args):
        ...
        for occ in args:
            ...
            if not type(occ) in (list, tuple):
                obj = [occ,]
            for rep in obj:
                if type(rep) not in (str, unicode):
                    continue
                if not self.run.Exists(rep):
                    self._mess(ufmt(_(u'path does not exist : %s'), rep),
                        '<E>_FILE_NOT_FOUND')
...
class AsterBuildInstalledNoCopy(AsterBuild_Classic):
    """For a version installed by waf and with enhancements to directly the
...
class AsterBuildInstalledNoSuperv(AsterBuildInstalledNoCopy):
    """For a version installed by waf, with enhancements to directly the
...
def AsterBuild(run, conf):
    ...
    for feature in ('use_numthreads', 'orbinitref'):
...
def getDiag(run, err='fort.9', resu='fort.8', mess='fort.6', cas_test=False):
    """Return the diagnostic after a Code_Aster execution
    as a list : [diag, tcpu, tsys, ttotal, telapsed]
        err   : error file
        resu  : result file (None = ignored)
